...
__global__ void computeSumKernel(const float* TAB, float* OUT, int N, int R, int OUT_size) {
...
cudaError_t computeSumGPU(const float* TAB, float* OUT, int N, int R) {
    ...
    dim3 blocksPerGrid((OUT_size + threadsPerBlock.x - 1) / threadsPerBlock.x,
        (OUT_size + threadsPerBlock.y - 1) / threadsPerBlock.y);

    computeSumKernel <<<blocksPerGrid, threadsPerBlock>>> (d_TAB, d_OUT, N, R, OUT_size);
